...
qboolean intInUse[16] = {false, false, false, false, false, false, false, false, false, false, false, false, false, false, false, false};
...
static void ComPort_Enable(ComPort *p)
{
	...
	if (intInUse[p->irq])
	{
		Con_Printf("IRQ%u is already in use\n", p->irq);
		return;
	}
...
static char *Modem_Response(ComPort *p)
{
	...
	while (! EMPTY(p->inputQueue))
	{
		...
		if (b == '\r')
		{
			if (p->modemResponsePrinted)
				continue;
			p->buffer[p->bufferUsed] = 0;
			Con_Printf("%s\n", p->buffer);
			p->modemResponsePrinted = true;
			SCR_UpdateScreen ();
			continue;
		}
		p->modemResponsePrinted = false;
