...
//	transformed[2] = DotProduct(local, r_ppn);		
	...
	fmul	%st(1)		// dot0 | local[0] | local[1] | local[2]
	flds	C(r_ppn)+4	// r_ppn[1] | dot0 | local[0] | local[1] | local[2]
	fmul	%st(3)		// dot1 | dot0 | local[0] | local[1] | local[2]
	flds	C(r_ppn)+8	// r_ppn[2] | dot1 | dot0 | local[0] |
						//  local[1] | local[2]
	fmul	%st(5)		// dot2 | dot1 | dot0 | local[0] | local[1] | local[2]
...
//		return;
	...
	fmul	%st(2)		// dot0 | local[2] | local[0] | local[1] | 1/z 
...
//	transformed[1] = DotProduct(local, r_pup);
	fmul	%st(4)		// dot1 | dot0 | local[2] | local[0] | local[1] | 1/z 
	flds	C(r_pup)+8	// r_pup[2] | dot1 | dot0 | local[2] |
						//  local[0] | local[1] | 1/z 
	fmul	%st(3)		// dot2 | dot1 | dot0 | local[2] | local[0] |
						//  local[1] | 1/z 
...
// FIXME: preadjust xcenter and ycenter
	...
	fxch	%st(2)			// 1/z | v | u
	fmuls	DP_0x8000		// 1/z * 0x8000 | v | u
