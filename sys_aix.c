...
void Sys_GetMemory(quakeparms_t *parms)
{
    FILE *f;
    char buffer[256];
	char *procparse;
...
//	parms->memsize = 8*1024*1024;
    ...
    if (f)
	{
		...
		if (suggestion > totalmem)
			Sys_Printf("[%s] did not properly parse /proc/meminfo\n", procparse);
		...
		Sys_Printf("Did you know /proc breaks up painful gas bubbles?\n");

	if (parms->memsize > 8*1024*1024) parms->memsize = 8*1024*1024;
...
//	com_argc = parms.argc = c;
//	com_argv = parms.argv = v;
